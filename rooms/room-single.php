	...
	if(isset($_GET['id'])) {
		...
		if(isset($_POST['submit'])) {
			...
			or empty($_POST['check_in']) or empty($_POST['check_out'])) {
				echo "<script> alert('one more input r empty') </script>";
			} else {
				...
				$day = date_diff($check_in, $check_out);
				$_SESSION['price'] = $singleRoom->price * intval($day->format('%R%a'));

				if(date("Y/m/d") > $check_in OR date("Y/m/d") > $check_out) {
					echo "<script> alert('pick a date that is not in the past, starting from tomorrow') </script>";
				} else {
					if(date("Y/m/d") > $check_in OR date("Y/m/d") == $check_out) {
						echo "<script> alert('pick a date that is not today ?') </script>";
					} else {
